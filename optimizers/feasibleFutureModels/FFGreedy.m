...
classdef FFGreedy < FeasibleFuture
    properties(Constant)
        tolerance = 1e-6
        verbose_top = false
        verbose = false
    ...
    methods
        function obj = FFGreedy(hashSize, nSegments, maxSize, thr, thr_down,...
            ttl, ttl_down, nt, nr)
        ...
        function [final, new] = newFeasibleFuture(obj, initialSet, timeSlot, dt,...
			...
			Rl = FFGreedy.calculateLoadResistances(Q0, deviceData, chargeData);
			
			%calculating the minimal receiving current to keep alive
			minIr = FFGreedy.calculateMinIr(Q0, deviceData, chargeData,...
				timeSlot.Id, dt);

			if FFGreedy.verbose_top
			...
			while consecutive_failures < obj.thr && attempts < obj.ttl &&...
				...
				[minK, maxK] = FFGreedy.calculateLimitConstants(v_base,i_base,...
					minIr, constraints);
				...
				if minK + FFGreedy.tolerance > maxK - FFGreedy.tolerance
					%the range is empty
					...
					for K = linspace(max(maxK - FFGreedy.tolerance, minK + FFGreedy.tolerance),...
						min(maxK - FFGreedy.tolerance, minK + FFGreedy.tolerance), obj.ttl_down)
						...
						Q = FFGreedy.integrateCharge(Q0,I,timeSlot.Id,deviceData,dt,chargeData);
						
						if FFGreedy.verbose_down
							print_vector('K', K, 2);
							print_vector('V', V, 2);
            ...
            %build the object
            new = FFGreedy(obj.hashSize, obj.nSegments, obj.maxSize, obj.thr,...
        ...
        function initial = generateInitialSet(obj, chargeData)
            ...
            %build the object
            initial = FFGreedy(1, obj.nSegments, 1, obj.thr, obj.thr_down,...
    ...
    methods(Static)
        ...
        function minIr = calculateMinIr(Q0, deviceData, chargeData, Id, dt)
            ...
            %tolerance value
            minIr = minIr + FFGreedy.tolerance;
        ...
        function [minK, maxK] = calculateLimitConstants(v_base,i_base,...
            ...
            maxK = sqrt((constraints.maxPact - FFGreedy.tolerance)/real((it_base')*v_base));

            for i=1:length(i_base)
                %the maximum k which respects the current constraint for element i
                maxK = min(maxK,(constraints.maxCurr(i) - FFGreedy.tolerance)/abs(i_base(i)));
            end
        ...
        function Q = integrateCharge(Q0, I, Id, deviceData, dt, chargeData)
            ...
            for r=1:length(Id)
                ...
                %final charge
                Q(r) = min(ic(r)*dt+Q0(r), chargeData.maximum(r)-FFGreedy.tolerance);
				Q(r) = max(0, Q(r));
            end
