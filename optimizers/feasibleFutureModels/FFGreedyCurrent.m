...
classdef FFGreedyCurrent < FeasibleFuture
    properties(Constant)
        tolerance = 1e-6
        verbose_top = false
        verbose = false
    ...
    methods
        function obj = FFGreedyCurrent(hashSize, nSegments, maxSize, ttl, nt, nr)
        ...
        function [final, new] = newFeasibleFuture(obj, initialSet, timeSlot, dt,...
			...
			best_Q = zeros(obj.nr,1);
			best_D0 = zeros(obj.nr,1);
			best_V = zeros(obj.nt,1);
			best_IR = -ones(obj.nr,1);%default: invalid
			...
			if FFGreedyCurrent.verbose_top
			...
			while attempts < obj.ttl && ~threshold_reached
				...
				if minK + FFGreedyCurrent.tolerance <= maxK - FFGreedyCurrent.tolerance
					%-K is not required to be tested, since it leads to the same abs(ir)

					V = (maxK - FFGreedyCurrent.tolerance)*v_base;
					I = (maxK - FFGreedyCurrent.tolerance)*i_base;

					Q = FFUtils.integrateCharge(Q0,I,timeSlot.Id,deviceData,dt,chargeData);
						
					if FFGreedyCurrent.verbose_down
						print_vector('K', maxK - FFGreedyCurrent.tolerance, 2);
						print_vector('V', V, 2);
					...
					if sum(abs(I(obj.nt+1:end)))>sum(best_IR)
						best_Q = Q;
						best_D0 = D0;
						best_V = V;
						best_IR = abs(I(obj.nt+1:end));
					end
            ...
            %build the object
            new = FFGreedyCurrent(obj.hashSize, obj.nSegments, obj.maxSize, obj.ttl, obj.nt, obj.nr);
        ...
        function initial = generateInitialSet(obj, chargeData)
            ...
            %build the object
            initial = FFGreedyCurrent(1, obj.nSegments, 1, obj.ttl, obj.nt, obj.nr);
